...
2. 암호화가 될떄 비번이 똑같을 시 DB에서 똑같은 해시값을 주어지기 때문에 암호를 만들때 조건을 만들어서 만들게 하는 것이다.
   정상적인 해시값은 다 소문자이다.(직접 바꾸거나 user table을 다 날리고 다시 만들어야 함)

3. mysql 서버 연결-비번 연결-save-user table(정보들)을 날릴 수 있음.

4. Index.html → main_index.html 이름을 수정이유: @Path("/") // 기본 경로가 최상위 / 이 부분에서 ("/") 최상위를 써서 로그인 유무를 먼저 판단하게 하기 위해서 수정하였음
